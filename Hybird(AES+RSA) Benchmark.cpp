...
// ================================================================
//  Benchmark Engine
//  - 데이터 크기별 AES / RSA / 하이브리드 성능을 100회 반복 측정
//  - 결과를 CSV로 저장 → Python으로 시각화
// ================================================================
...
private:
    ...
    long long measure_avg_us(Func f, int repeat) {
        long long total = 0;
...
public:
    void run() {
        std::cout << "\n========================================\n";
        std::cout << "   성능 벤치마크 (데이터 크기별 측정)  \n";
        std::cout << "========================================\n";

        // RSA 키 생성 (벤치마크 전체에서 동일한 키 사용)
        rsa.generate_keys();
        aes.set_key(session_key);

        // 세션키 RSA 암호화는 키가 고정이므로 한 번만 수행
        std::vector<long long> enc_blocks;
        rsa.encrypt_aes_key(session_key, enc_blocks);

        // 테스트 데이터셋
        std::vector<std::pair<std::string, std::vector<unsigned char>>> test_cases;
        auto make_data = [](size_t n) { return std::vector<unsigned char>(n, 'A'); };

        test_cases.push_back({ "2B",   {'H','I'} });
        test_cases.push_back({ "10B",  make_data(10) });
        test_cases.push_back({ "50B",  make_data(50) });
        test_cases.push_back({ "100B", make_data(100) });
        test_cases.push_back({ "500B", make_data(500) });
        test_cases.push_back({ "1KB",  make_data(1024) });
        test_cases.push_back({ "5KB",  make_data(5 * 1024) });
        test_cases.push_back({ "10KB", make_data(10 * 1024) });

        const int REPEAT = 100;

        std::ofstream csv("benchmark_result.csv");
        csv << "label,size_byte,aes_enc_us,aes_dec_us,rsa_key_enc_us,rsa_key_dec_us,hybrid_total_us\n";

        std::cout << "\n반복 횟수: " << REPEAT << "회 평균\n";
        std::cout << "--------------------------------------------------------------------\n";
        std::printf("%-8s %8s %10s %10s %13s %13s %15s\n",
            "레이블", "크기(B)", "AES암호화", "AES복호화",
            "RSA키암호화", "RSA키복호화", "하이브리드합계");
        std::cout << "--------------------------------------------------------------------\n";

        for (auto& tc : test_cases) {
            const std::string& label = tc.first;
            const std::vector<unsigned char>& data = tc.second;

            std::vector<unsigned char> cipher;
            long long aes_enc = measure_avg_us([&]() { cipher = aes.encrypt(data); }, REPEAT);
            long long aes_dec = measure_avg_us([&]() { aes.decrypt(cipher); }, REPEAT);

            std::vector<long long> tmp_enc;
            long long rsa_enc = measure_avg_us([&]() {
                rsa.encrypt_aes_key(session_key, tmp_enc);
                }, REPEAT);

            unsigned char tmp_key[32] = { 0 };
            long long rsa_dec = measure_avg_us([&]() {
                rsa.decrypt_aes_key(enc_blocks, tmp_key);
                }, REPEAT);

            long long hybrid_total = rsa_enc + aes_enc + rsa_dec + aes_dec;

            std::printf("%-8s %8zu %10lld %10lld %13lld %13lld %15lld\n",
                label.c_str(), data.size(),
                aes_enc, aes_dec, rsa_enc, rsa_dec, hybrid_total);
        ...
        csv.close();
        std::cout << "--------------------------------------------------------------------\n";
        std::cout << "CSV 저장 완료: benchmark_result.csv\n";
        std::cout << "========================================\n";
    }
};
